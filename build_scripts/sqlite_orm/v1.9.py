...
if __name__ == "__main__":
    ...
    record_step_file_name = "build_log/sqlite_orm-v1.9.ini"
    ...
    steps: list[tuple[str, Dict[str, str]]] = [
        ("get_source_code", {   "work_path"  : "sheer_third_party",
                                "clone_path" : "libraries/sqlite_orm-src",
                                "url"        : "https://github.com/fnc12/sqlite_orm.git",
                                "version"    : "v1.9"}),
        ("pre_build", { "work_path"        : "sheer_third_party",
                        "source_path"      : "libraries/sqlite_orm-src",
                        "build_dir"        : "build",
                        "version"          : "v1.9",
                        "pre_install_path" : "libraries/sqlite_orm",
                        "sqlite_path"      : "libraries/sqlite-3.45.0",
                        "cmake_options1"   : "-DCMAKE_INSTALL_PREFIX=",
                        "cmake_options2"   : "-DSQLite3_INCLUDE_DIR=",
                        "cmake_options3"   : "-DSQLite3_LIBRARY="}),
    ...
    try:
        ...
        for step_name, step_data in steps:
            ...
            if step_id == 1:
                ...
                build_log.set_value(step_name, "sqlite_path",      step_data["sqlite_path"])
                build_log.set_value(step_name, "cmake_options1",   step_data["cmake_options1"])
                build_log.set_value(step_name, "cmake_options2",   step_data["cmake_options2"])
                build_log.set_value(step_name, "cmake_options3",   step_data["cmake_options3"])
                # 执行本步骤
                ...
                cmake_param2: str = step_data["cmake_options2"] + current_path + "/" + step_data["sqlite_path"] + "/include"
                cmake_param3: str = step_data["cmake_options3"] + current_path + "/" + step_data["sqlite_path"] + "/lib/libsqlite3.so"
                if pre_build(step_data["work_path"], step_data["source_path"], step_data["build_dir"], cmake_param1, cmake_param2, cmake_param3, "..") == False:
                    change_path_to(current_path)

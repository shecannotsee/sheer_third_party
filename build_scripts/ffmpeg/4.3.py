...
if __name__ == "__main__":
    ...
    record_step_file_name = "build_log/ffmpeg-4.3.ini"
    ...
    steps: list[tuple[str, Dict[str, str]]] = [
        ...
        ("pre_build", {"work_path"         : "sheer_third_party",
                       ...
                       "configure_options6": "--enable-ffplay"}),
    ...
    try:
        ...
        for step_name, step_data in steps:
            ...
            if step_id == 1:
                ...
                build_log.set_value(step_name, "configure_options6", step_data["configure_options6"])
                # 执行本步骤
                ...
                if pre_build(step_data["work_path"], step_data["source_path"],
                             ...
                             step_data["configure_options5"],
                             step_data["configure_options6"]) == False:
                    change_path_to(current_path)
                    raise Exception(f"Step execution interrupt: {step_name}")
                # 执行成功则记录步骤执行结果
                change_path_to(current_path)

...
if __name__ == "__main__":
    ...
    record_step_file_name = "build_log/PostgreSQL-14.12.ini"
    ...
    steps: list[tuple[str, Dict[str, str]]] = [
        ("get_source_code", {"work_path" : "sheer_third_party",
                             "clone_path": "libraries/PostgreSQL-source",
                             "url"       : "https://github.com/postgres/postgres.git",
                             "version"   : "REL_14_12"}),
        ("pre_build", {"work_path"         : "sheer_third_party",
                       "source_path"       : "libraries/PostgreSQL-source",
                       "version"           : "14.12",
                       "pre_install_path"  : "libraries/PostgreSQL",
    ...
    try:
        ...
        for step_name, step_data in steps:
            ...
            if step_id == 1:
                ...
                if pre_build(step_data["work_path"], step_data["source_path"], configure_param1) == False:
                    change_path_to(current_path)
                    raise Exception(f"Step execution interrupt: {step_name}")
                # 执行成功则记录步骤执行结果
                change_path_to(current_path)
    ...
    record(LOG_LEVEL.INFO, "build and install lib: PostgreSQL-14.12 success")
